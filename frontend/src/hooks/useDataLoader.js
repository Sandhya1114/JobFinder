...
export const useDataLoader = () => {
  ...
  // Load dynamic filter options from backend using dedicated API endpoint
  const loadFilterOptions = useCallback(async () => {
    try {
      console.log('📊 Loading dynamic filter options from backend API...');
      
      // Use the dedicated backend endpoint for better performance
      const response = await api.fetchFilterOptions();
      ...
      const options = {
        ...
        salaryRanges: response.salaryRanges || [],
        isLoaded: true,
        stats: response.stats || {}
      };
      
      console.log('✅ Dynamic filter options loaded from backend:', options.stats);
  ...
  const loadAllData = useCallback(async () => {
    try {
      // Load categories, companies, and filter options in parallel
      await Promise.all([
        loadCategories(),
        loadCompanies(),
        loadFilterOptions()
  ...
  }, [loadJobs, loadCategories, loadCompanies, loadFilterOptions, dispatch]);
  ...
  return {
    ...
    refreshJobs,
    resetInfiniteScroll,
    filterOptions // Export dynamic filter options
  };
};