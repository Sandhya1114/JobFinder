...
//   //   navigate(newURL, { replace: true });
//   // }, [dispatch, showToast, navigate, location.pathname, filters.searchQuery]);
//   // 4. FIXED: Update handleClearFilters to properly clear everything including search
// const handleClearFilters = useCallback(() => {
//   console.log('Clearing all filters');
...
// //       // Check if removed value matches search input (case-insensitive)
// //       if (localFilters.searchInput.toLowerCase().includes(value.toLowerCase())) {
// //         // Clear search input and Redux search query
...
// //     };
// //   });
// // }, [dispatch, localFilters.searchInput]);
// // const handlePendingFilterChange = useCallback((filterType, value, isChecked) => {
// //   console.log('Filter change:', filterType, value, isChecked);
...
// //   });
// // }, []);
//  / 3. FIXED: Update handlePendingFilterChange to properly handle multiple selections
// const handlePendingFilterChange = useCallback((filterType, value, isChecked) => {
//   console.log('Filter change:', filterType, value, isChecked);
...
// //   // Check if search input contains any cleared value (case-insensitive)
// //   const searchInputLower = localFilters.searchInput.toLowerCase();
// //   const shouldClearSearch = clearedValues.some(val => searchInputLower.includes(val.toLowerCase()));
...
// //   showToast(`${filterType.replace('selected', '')} filter cleared!`);
// // }, [dispatch, pendingFilters, localFilters.searchInput, showToast]);
//    // 2. FIXED: Update handleDropdownClear to properly clear related search terms
// const handleDropdownClear = useCallback((filterType) => {
//   console.log('Clearing single filter:', filterType);
...
//     case 'selectedCategory':
//       dispatch(setSelectedCategory([]));
//       // Check if search query matches any cleared category and clear it
//       if (clearedValues.length > 0) {
//         const searchLower = localFilters.searchInput.toLowerCase();
//         const shouldClearSearch = categories.some(cat => {
//           if (clearedValues.includes(cat.id)) {
//             return searchLower.includes(cat.name.toLowerCase()) || 
...
//     case 'selectedCompany':
//       dispatch(setSelectedCompany([]));
//       // Check if search query matches any cleared company and clear it
//       if (clearedValues.length > 0) {
//         const searchLower = localFilters.searchInput.toLowerCase();
//         const shouldClearSearch = companies.some(comp => {
//           if (clearedValues.includes(comp.id)) {
//             return searchLower.includes(comp.name.toLowerCase()) || 
...
//   showToast(`${filterType.replace('selected', '')} filter cleared!`);
// }, [dispatch, pendingFilters, categories, companies, localFilters.searchInput, showToast]);
//   // Modal handlers
//   const handleViewDetails = useCallback((job) => {
//     setSelectedJob(job);
//     setIsModalOpen(true);
...
//   // FIXED: Search term to filter mapping helper
...
//           term.includes(categoryName) ||
//           categoryName.split(' ').some(word => term.includes(word) && word.length > 2)) {
...
//           term.includes(companyName) ||
//           companyName.split(' ').some(word => term.includes(word) && word.length > 2)) {
...
//   // FIXED: Enhanced debounce search input with smart filter mapping
//   useEffect(() => {
//     const debounceTimer = setTimeout(() => {
//       if (localFilters.searchInput !== filters.searchQuery) {
...
//         // FIXED: Smart filter mapping - automatically add related filters when searching
//         if (newQuery && newQuery.trim()) {
//           const relatedFilters = getRelatedFiltersFromSearch(newQuery.trim());
          
//           // Add related categories to pending filters
...
//           // Add related companies to pending filters
...
//     return () => clearTimeout(debounceTimer);
//   }, [localFilters.searchInput, filters.searchQuery, dispatch, getRelatedFiltersFromSearch, filters.selectedCategory, filters.selectedCompany]);
...
//   // FIXED: Enhanced filter handlers with proper multiple selection support
//   const handleApplyFilters = useCallback(() => {
//     console.log('Applying filters:', pendingFilters);
    
//     // Apply all pending filters at once - ALLOWS MULTIPLE SELECTIONS
//     dispatch(setSelectedCategory(pendingFilters.selectedCategory));
//     dispatch(setSelectedCompany(pendingFilters.selectedCompany));
//     dispatch(setSelectedExperience(pendingFilters.selectedExperience));
//     dispatch(setSelectedLocation(pendingFilters.selectedLocation));
...
//     const totalFiltersApplied = Object.values(pendingFilters).reduce((acc, arr) => acc + arr.length, 0);
...
//   // FIXED: Enhanced pending filter change handler with proper multiple selection support
//   const handlePendingFilterChange = useCallback((filterType, value, isChecked) => {
//     console.log('Filter change:', filterType, value, isChecked);
